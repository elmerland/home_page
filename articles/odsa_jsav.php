<!DOCTYPE html>
<html>
<head>
	<?php
		require_once("../php/mainCSS.php");
		require_once("../php/articleCSS_JS.php");
	?>
	<title>OpenDSA: JSAV</title>
</head>
<body>
	<?php
		require_once("../php/social/fbJS.php");
		require_once("../php/header.php");
		require_once("../php/nav.php");
	?>

	<div class="banner" style="background-image: url(../_images/patterns/leather-nunchuck.png);"></div>

	<article class="main">
		
		<nav class="outline float">
			<section>
				<a class="toTop" href="#"><h3>Back to top</h3></a>
				<h2>Outline</h2>
				<ul>
					<a href="#intro"><li>Intro</li></a>
					<a href="#js1"><li>JS: Getting Started</li></a>
					<a href="#js2"><li>JS: Encapsulation</li></a>
					<a href="#js3"><li>JS: Abs. Positioning</li></a>
					<a href="#css1"><li>CSS: Getting Started</li></a>
					<a href="#css2"><li>CSS: Container Size</li></a>
					<a href="#css3"><li>CSS: Node Size</li></a>
					<a href="#css4"><li>CSS: Text Alignment</li></a>
					<a href="#notes"><li>Notes</li></a>
					<a href="#disqus_thread"><li>Comments</li></a>
				</ul>
			</section>
		</nav>

		<div class="content">

			<header>
				<h1>OpenDSA: Getting Started With JSAV</h1>
				<p><time pubdate datetime="2014-02-02">Created on: Feburary 2, 2014</time></p>
				<section class="social">
					<?php
						include("../php/social/fbLike.php");
						include("../php/social/twitter.php");
					?>
				</section>
			</header>
			
			<section id="intro">
				<header>
					<h2>Intro</h2>
					<a href="#intro"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>

			<section id="js1">
				<header>
					<h2>JavaScript: Getting Started</h2>
					<a href="#js1"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>

			<section id="js2">
				<header>
					<h2>JavaScript: Encapsulation</h2>
					<a href="#js2"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>

			<section id="js3">
				<header>
					<h2>JavaScript: Absolute Positioning</h2>
					<a href="#js3"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>

			<section id="css1">
				<header>
					<h2>CSS: Getting Started</h2>
					<a href="#css1"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>

			<section id="css2">
				<header>
					<h2>CSS: Container Size</h2>
					<a href="#css2"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>

			<section id="css3">
				<header>
					<h2>CSS: Node Size</h2>
					<a href="#css3"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>

			<section id="css4">
				<header>
					<h2>CSS: Text Alignment</h2>
					<a href="#css4"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
			
				<p>
				</p>
			</section>
	
			<section id="notes">
				<header>
					<h2>Notes</h2>
					<a href="#notes"><img src="../_images/link20.png"></a>
					<a class="toTop" href="#"><img src="../_images/toTop.png"></a>
				</header>
		
				<p>
				</p>
			</section>
			<section class="social">
				<?php
					include("../php/social/fbLike.php");
					include("../php/social/twitter.php");
				?>
			</section>
			
			<footer>
				<p>
					Written by: Elmer Landaverde <a href="mailto:elmerlandaverde@gmail.com">(elmerlandaverde@gmail.com)</a><br>
					Last updated: February 2, 2014<br>
					<a title="Nerdfighters" href="http://nerdfighters.ning.com/" target="_blank">DFTBA!</a>
				</p>
			</footer>
			<?php require_once("../php/disqus.php"); ?>
		
		</div>
	</article>
</body>
</html>
